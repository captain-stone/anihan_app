    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `login_credential_tbl` (`id` INTEGER NOT NULL, `refreshToken` TEXT, `displayName` TEXT, `uid` TEXT, `username` TEXT, `password` TEXT, `token` TEXT NOT NULL, `dateLogin` TEXT, PRIMARY KEY (`id`))');
...
class _$LoginDao extends LoginDao {
  ...
  @override
  Future<LoginEntity?> getLoginDao() async {
    return _queryAdapter.query('SELECT * FROM login_credential_tbl',
        mapper: (Map<String, Object?> row) => LoginEntity(
            row['token'] as String,
            displayName: row['displayName'] as String?,
            refreshToken: row['refreshToken'] as String?,
            uid: row['uid'] as String?,
            dateLogin: row['dateLogin'] as String?,
  ...
  Future<void> deleteData() async {
    await _queryAdapter.queryNoReturn('DELETE FROM login_credential_tbl');
  }
